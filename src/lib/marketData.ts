...
class MarketDataServiceImpl {
  ...
  private static async fetchAlphaQuote(symbol: string): Promise<LiveQuote> {
    ...
    try {
      ...
      console.error('Failed to parse Alpha Vantage quote response:', error);
      throw new Error('Alpha Vantage returned an invalid response.');
    }

    const result = (payload as { 'Global Quote'?: Record<string, unknown> })?.['Global Quote'];
  ...
  private static async fetchAlphaIntraday(symbol: string): Promise<HistoricalPricePoint[]> {
    ...
    try {
      ...
      console.error('Failed to parse Alpha Vantage intraday response:', error);
      throw new Error('Alpha Vantage returned an invalid response.');
    }

    const seriesKey = `Time Series (${this.ALPHA_INTRADAY_INTERVAL})`;
    const series = (payload as Record<string, unknown>)[seriesKey] as Record<string, Record<string, unknown>> | undefined;
  ...
  private static async fetchAlphaDaily(
    ...
    try {
      ...
      console.error('Failed to parse Alpha Vantage daily response:', error);
      throw new Error('Alpha Vantage returned an invalid response.');
    }

    const series = (payload as { 'Time Series (Daily)'?: Record<string, Record<string, unknown>> })?.['Time Series (Daily)'];
